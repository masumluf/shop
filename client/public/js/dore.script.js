...
$.dore = function (element, options) {
  ...
  function init() {
    ...
    function setMenuClassNames(clickIndex, calledFromResize, link) {
      ...
      //menu-sub-hidden no subpage
      if (
        $(".menu ul[data-link='" + link + "']").length == 0 &&
        (menuClickCount == 1 || calledFromResize)
      ) {
        if ($(window).outerWidth() >= menuHiddenBreakpoint) {
    ...
    if ($().DataTable) {
      $(".data-table-standard").DataTable({
        ...
        info: false,
        sDom:
          '<"row view-filter"<"col-sm-12"<"float-left"l><"float-right"f><"clearfix">>>t<"row view-pager"<"col-sm-12"<"text-center"ip>>>',
        pageLength: 6,
        language: {
          paginate: {
      ...
      $(".data-table-feature").DataTable({
        sDom:
          '<"row view-filter"<"col-sm-12"<"float-right"l><"float-left"f><"clearfix">>>t<"row view-pager"<"col-sm-12"<"text-center"ip>>>',
        columns: [
          { data: "name" },
          { data: "position" },
          { data: "office" },
    ...
    $("#exampleModalContent").on("show.bs.modal", function (event) {
      ...
      modal
        .find(".modal-title")
        .text("Please Enter Your Contact Details below.");
    ...
    if ($().smartWizard) {
      ...
      // Validation
      $("#smartWizardValidation").on("showStep", function (
        e,
        anchorObject,
        stepNumber,
        stepDirection,
      ...
      $("#smartWizardValidation").on("leaveStep", function (
        e,
        anchorObject,
        stepNumber,
        stepDirection,
